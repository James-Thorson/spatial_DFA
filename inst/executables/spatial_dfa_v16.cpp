...
Type objective_function<Type>::operator() ()
{
  ...
  for(int j=0; j<n_factors; j++){
    // Omega
    if( Options_vec(5)==0 ) Q = Q_spde(spde, exp(logkappa_jz(j,0)) );
    if( Options_vec(5)==1 ) Q = Q_spde(spde_aniso, exp(logkappa_jz(j,0)), H);
    ...
    if( Options_vec(5)==0 ) Q = Q_spde(spde, exp(logkappa_jz(j,1)) );
    if( Options_vec(5)==1 ) Q = Q_spde(spde_aniso, exp(logkappa_jz(j,1)), H);
