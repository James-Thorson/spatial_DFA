...
Type objective_function<Type>::operator() ()
{
  ...
  for(int j=0; j<n_factors; j++){
    ...
    logtau_O_j(j) = 0.5*( log(1.0+lambda_j(j)) - log(4.0*pi*square(kappa_j(j))*square(1-rho_j(j))) );
    logtau_E_j(j) = logtau_O_j(j) + log(1-rho_j(j)) - 0.5*log( lambda_j(j) * (1-square(rho_j(j))) );
    VarSpace_j(j) = 1.0 / ( 4.0*pi*exp(2.0*logtau_O_j(j))*exp(2.0*logkappa_j(j)) * square(1-rho_j(j)) );
    VarTime_j(j) = 1.0 / ( 4.0*pi*exp(2.0*logtau_E_j(j))*exp(2.0*logkappa_j(j)) * (1-square(rho_j(j))) );
  }
