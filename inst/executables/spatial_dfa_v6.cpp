...
Type objective_function<Type>::operator() ()
{
  ...
  for(int j=0; j<n_factors; j++){
    ...
    logtau_O_j(j) = 0.5*( log(1.0+lambda_j(j)) - log(4.0*pi*square(kappa_j(j))) );
    logtau_E_j(j) = logtau_O_j(j) - 0.5*log( lambda_j(j) * (1-square(rho_j(j))) );
  ...
  jnll += pen_epsilon_j.sum();
  jnll += pen_omega_j.sum();
