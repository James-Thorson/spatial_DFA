...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_MATRIX( X_nl );		    // Spatial-Covariate design matrix
  ...
  PARAMETER_MATRIX(gamma_lp);  // Density-covariates
  ...
  matrix<Type> eta_np(n_knots, n_species);
  ...
  for(int i=0; i<n_obs; i++){
    // Expectation
    logchat_i(i) = eta_np(s_i(i),p_i(i)) + eta_i(i) + theta_npt(s_i(i),p_i(i),t_i(i));
    // Overdispersion
  ...
  if( a_n.sum() > 0 ){
    ...
    for(int p=0; p<n_species; p++){
      for(int n=0; n<n_knots; n++){
        Index_npt(n,p,t) = exp(eta_np(n,p) + theta_npt(n,p,t)) * a_n(n) / 1000;  // Convert from kg to metric tonnes
