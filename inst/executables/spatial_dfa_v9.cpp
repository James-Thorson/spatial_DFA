  ...
  for(int i=0; i<n_obs; i++){
    ...
    if( !isNA(c_i(i)) ){                
      ...
      if( Options_vec(0)!=0 ){ 
        ...
        if( Options_vec(3)==2 ){
          encounterprob_i(i) = plogis(zinfl_pz(p_i(i),1)) * ( 1.0 - exp(-1 * exp(logchat_i(i)) * exp(zinfl_pz(p_i(i),0))) );
        }
        // probability of data
        if( Options_vec(0)==1 ) nll_i(i) = -1 * dzinflognorm(c_i(i), logchat_i(i)-log(encounterprob_i(i)), encounterprob_i(i), exp(log_sigma_p(p_i(i))), true);
        if( Options_vec(0)==2 ) nll_i(i) = -1 * dzinfgamma(c_i(i), exp(logchat_i(i))/encounterprob_i(i), encounterprob_i(i), exp(log_sigma_p(p_i(i))), true);
        if( Options_vec(0)==3 ) nll_i(i) = -1 * dzinfnorm(c_i(i), exp(logchat_i(i))/encounterprob_i(i), encounterprob_i(i), exp(logchat_i(i))/encounterprob_i(i)*exp(log_sigma_p(p_i(i))), true);
      }
    }
  }
  jnll_c(3) = nll_i.sum();
