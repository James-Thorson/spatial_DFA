...
Type objective_function<Type>::operator() ()
{
  ...
  array<Type> mu_sjt(n_knots,n_factors,n_years); 
  vector<Type> eta_i(n_obs);
  array<Type> logchat_spt(n_knots,n_species,n_years);
  
  // Computate factors
  for(int s=0; s<n_sites; s++){
  for(int j=0; j<n_factors; j++){
  for(int t=0; t<n_years; t++){
    mu_sjt(s,j,t) = phi_j(j)*pow(rho_j(j),t) + Epsilon(s,j,t) + ( alpha_j(j) + Omega(s,j) )/(1-rho_j(j));
  }}}
  ...
  for(int p=0; p<n_species; p++){
  for(int t=0; t<n_years; t++){
    logchat_spt(s,p,t) = 0.0;
    for(int j=0; j<n_factors; j++){
      logchat_spt(s,p,t) += mu_sjt(s,j,t) * Psi_jp(j,p);
    }
  }}}
  ...
  for(int i=0; i<n_obs; i++){
    // Expectation
    logchat_i(i) = eta_i(i) + logchat_spt(s_i(i),p_i(i),t_i(i));
    // Overdispersion
    logchat_i(i) += delta_i(i) * exp(log_sigma_p(p_i(i)));
    // Likelihood
    if( !isNA(c_i(i)) ){                
